...
can be achieved.
Without DBSCAN, k-means clustering will likely assing one cluster to outliers
of faulty data.
...
\begin{figure}[htbp]
  ...
  \caption{Steps of the proposed framework for predicting of transportation
    modes by Etemad et al.~\cite{etemad_predicting_2018}}
...
some trackings showed GPS tracks only on the parking facility and not in
operation,
while others showed that the tracking device being turned on over night, while
no
operation was taking place either.
